...
#include "engine.h"
// position fen rnbqkbnr/ppp1pppp/8/3p4/5P2/8/PPPPP1PP/RNBQKBNR w KQkq - 0 1 moves e2e3 d5d4 e3e4 d4d3 f1d3 h7h5 g1f3
// h5h4 e1g1 h4h3 g2h3 e7e6 d3b5 c7c6 b5c4 b7b5 c4e2 b5b4 g1g2 f7f5 e4f5 e6f5 e2c4 g8h6 f3g5 g7g6 g5e6 d8d6 e6f8 h8f8
// c2c3 b4c3 d2c3 d6d1 f1d1 c8b7 c1e3 a7a5 e3c5 f8f6 d1e1 e8d7 e1e7 d7c8 b1d2 a5a4 a1d1 g6g5 f4g5 f6g6 c4e6 g6e6 e7e6
// a8a5 g5h6 a5c5 h6h7 c8d7 h7h8q d7e6 h8b8 b7a6 b8b6 a6e2 b6c5 e2d1 c5c6 e6e5 d2c4 e5f4 c6d6 f4g5 d6d1 a4a3 b2a3 f5f4
// d1g4 g5h6 g4f4 h6g6 h3h4 g6h7 h4h5 h7g7 f4f5 g7g8 h5h6 g8h8 f5g4 h8h7 c4e5 h7h8 h2h3 h8h7 h3h4 h7h8 c3c4 h8h7 a3a4
// h7h8 a2a3 h8h7 h4h5 h7h8 c4c5 h8h7 a4a5 h7h8 a3a4 h8h7 c5c6 h7h8 a5a6 h8h7 a4a5 h7h8 c6c7 h8h7 a6a7 h7h8 a5a6 h8h7
// c7c8n h7h8 a7a8n h8h7 a6a7 h7h8
...
int Engine::partition(uint16_t arr[], int start, int end) {
  ...
  for (int i = start + 1; i <= end; i++) {
    if (mvvlvaOrder(arr[i]) <= pivoteValue) count++;
  }
...
int Engine::search(int alpha, int beta, uint64_t attackers, int depthleft, int originalDepth, uint16_t& bestMove,
                   bool& left, int& totalNodes, bool wasNull, uint16_t prevBest, bool isPV, int pvIndex) {
  totalNodes++;
  bool wasCheckExtension = false;
  if (depthleft <= 0) {
    if (!(board->getKing() & attackers)) return quiesce(alpha, beta, attackers, totalNodes, 0);
    depthleft = 1;
  ...
  for (int i = 0; i < total; i++) {
    ...
    if (i == 0) {
      score =
          -search(-beta, -alpha, a, depthleft - 1, originalDepth, nbm, left, totalNodes, false, 0, isPV, pvIndex - 1);
    } else {
      score = -search(-alpha - 1, -alpha, a, depthleft - 1, originalDepth, nbm, left, totalNodes, false, 0, false,
                      pvIndex - 1);
      if (score > alpha && isPV) {
        score =
            -search(-beta, -alpha, a, depthleft - 1, originalDepth, nbm, left, totalNodes, false, 0, true, pvIndex - 1);
      }
    }
...
int Engine::quiesce(int alpha, int beta, uint64_t attackers, int& totalNodes, int depth) {
  ...
  std::chrono::duration<double, std::milli> ms_double = std::chrono::high_resolution_clock::now() - startClock;
  int elapsed = (int)(ms_double.count());
  if ((maxTime != -1 && elapsed >= maxTime - maxTime * 0.05)) return alpha;
...
uint16_t Engine::runSearchID(int m, int& score, int& nodeCount) {
  ...
  for (int i = 1; i < 10; i++) {
    ...
    int s =
        search(-100000, 100000, board->getAttackers(), i, i, bestMove, done, totalNodes, false, runningBest, true, 15);
